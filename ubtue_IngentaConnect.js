{
	...
	"lastUpdated": "2021-05-17 16:35:02"
}
...
function scrape(newDoc, url) {
	...
	Zotero.Utilities.HTTP.doGet(risurl, function (text) {
		...
		translator.setHandler("itemDone", function (obj, item) {
			...
			var issnTag = ZU.xpath(newDoc, '//a[@title="OpenURL link"]');
			for (let entry in issnTag) {
			var ISSN = issnTag[entry].href.match(/eissn=(\d{4}-.{4})&/);
			if (typeof ISSN[1] != 'undefined') {
			item.ISSN = ISSN[1];
			}
			break;
			}
