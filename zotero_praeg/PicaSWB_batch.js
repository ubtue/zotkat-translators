...
function performExport() {
	...
	while ((item = Zotero.nextItem())) {
        ...
        if (item.url && item.itemType == "magazineArticle") {
            addLine(currentItemId, "\\n4950", "$u" + item.url + "$xR"); //K10Plus:wird die URL aus dem DOI, einem handle oder einem urn gebildet, sollte es $xR heißen und nicht $xH
        }
        ...
        if (item.itemType == "journalArticle" || item.itemType == "magazineArticle") {
            if (superiorPPN.length != 0) {
                ...
                addLine(currentItemId, "\\n4241", "Enthalten in " + journal_title_to_ppn[item.publicationTitle]);
            }
