{
	...
	"lastUpdated": "2021-09-10 14:30:13"
}
...
function getORCID(doc, item) {
	let authorOrcidEntries = doc.querySelectorAll('#sb-1 span');//Z.debug(authorOrcidEntries)
	for (let authorOrcidEntry of authorOrcidEntries) {
		let authorEntry = authorOrcidEntry.querySelector('.author-name accordion-tabbed__control, span');Z.debug(authorEntry)
		...
		if (authorEntry && orcidEntry && orcidEntry.text && orcidEntry.text.match(/\d+-\d+-\d+-\d+x?/i)) {
			let author = authorEntry.textContent;Z.debug(author)
