{
	"translatorID": "9f0d4b9f-2a3c-467b-9e43-29080c1a424f",
	"label": "Innsbruck_BILDI_PicaK10plus_single",
...
function addLine(itemid, code, value) {
    ...
    var line = code + " " + value.replace('|s|RezensionstagPica', '').replace(/\t/g, '').replace(/\|s\|peer\s?reviewed?/i, '|f|Peer reviewed').replace(/\|s\|book\s+reviews?/i, '|f|Book Reviews').replace(' $d', '$d').replace('https://doi.org/https://doi.org/', 'https://doi.org/');
...
function performExport() {
	...
	while ((item = Zotero.nextItem())) {
		...
		addLine(currentItemId, "0575", "BIIN");
